                ...
                if let Some(actual) = v.strip_prefix(URL_ESCAPE) {
                    #[cfg(feature = "fetching")]
                    {
                        ...
                        if let DeserState::User(fs) = IS_USER_FACING.with_borrow(|x| x.clone()) {
                            ...
                            if !filename.exists() {
                                download_file(url, &filename, &fs).map_err(|x| {
                                    de::Error::custom(format!(
                                        "could not download file {v}...\n{x}"
                                    ))
                                })?;

                                fs.set_permissions(&filename, PERM).map_err(|x| {
                                    de::Error::custom(format!(
                                        "could set permissions for file {v}...\n{x}"
                                    ))
                                })?;
                            }
                    ...
                    {
                        Err(de::Error::custom(
                            "this version of gourd was built without fetching support, do not use urls",
                        ))
                    }
                } else {
