...
arguments = ["-f1", "./inputs/2.in", "-f2", "./input2/test.in"]
      ...
      \subsection{PARAMETERS}
        ...
        Parameters can be applied to the arguments to conveniently perform experiments with grid search.
            ...
            If an argument starts with \Arg{parameter|}, it will be treated as a parameter.
            For each value of that parameter the new input will be created with that value inserted into the argument
            into \Arg{\{parameter\_parameter-name\}}.

            Values of Parameter are specified in \Arg{[parameters.parameter-name]} using \Arg{values = []}
...
values = ["10", "20"]
    ...
    \subsubsection{SUBPARAMETERS}
        ...
        Subparameters are specified in inputs similarly to parameters with a difference of adding \Arg{\_subparameter-name}.
        This results in \Arg{\{parameter\_parameter-name\_subparameter-name\}}.

        Values of Subparameter are specified in \Arg{[parameters.parameter-name.sub.subparameter-name]} using \Arg{values = []}
